...
public class IRSystem {
  ...
  void index() {
    ...
    /**
     * TODO: Create an inverted index. Granted this may not be a linked list as
     * in a proper implementation. The 'documents' instance variable  will be
     * helpful.
     */
  ...
  void computeTFIDF() {
     ...
     * that you can make use of the following instance variables.
     *   - vocab
     *   - documents
     * Note that you probably do *not* want to store a value for every
     * word-document pair, but rather just for those pairs where a word actually
  ...
  public double getTFIDFUnstemmed(String word, int doc) {
    /**
     * This function gets the TF-IDF of an *unstemmed* word in a document. Stems
     * the word and then calls get_tfidf. You should *not* need to change this
     * interface, but it is necessary for submission.
     */
  ...
  ArrayList<Integer> booleanRetrieve(ArrayList<String> query) {
    /**
     * Given a query in the form of a list of *stemmed* words, this returns the
     * list of documents in which *all* of those words occur (ie an AND query).
     */

    /**
     * TODO: Implement Boolean retrieval. You will want to use your inverted
     * index that you created in index(). Right now this just returns all the
     * possible documents! Return an empty list if the query does not return any
     * documents.
     */
  ...
  PriorityQueue<Integer> rankRetrieve(ArrayList<String> query) {
    ...
    // TODO: Implement cosine similarity.  Starter code is Jaccard similarity.
  ...
  /**
   * Close a BufferedReader if possible, or do nothing if not.
   * @param br any BufferedReader
   */
  ...
  public static void runTests(IRSystem irSys) {
    ...
    BufferedReader inputQueries = null;
    try {
      inputQueries = new BufferedReader(new FileReader(new File(
          "../data/queries.txt")));
      questions = new ArrayList<String>();
      String line;
      while ((line = inputQueries.readLine()) != null) {
        questions.add(line);
      }
    } catch (IOException e) {
      System.err
          .println("Error reading ../data/queries.txt: " + e.getMessage());
    } finally {
      safeClose(inputQueries);
    }

    BufferedReader inputSols = null;
    try {
      inputSols = new BufferedReader(new FileReader(new File(
          "../data/solutions_java.txt")));
      solutions = new ArrayList<String>();
      String line;
      while ((line = inputSols.readLine()) != null) {
        solutions.add(line);
      }
    } catch (IOException e) {
